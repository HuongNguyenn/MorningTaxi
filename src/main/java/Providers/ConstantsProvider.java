package Providers;

public class ConstantsProvider {
	public static final String URL = "http://morning.taxionline.vn/";
	public static final String ChromeBrowser = "chrome";
	public static final String Firefox = "firefox";
	public static final String Ie = "ie";
	public static final String userName1 = "huongnguyen";
	public static final String passWord1 = "123456";
	public static final String userName2 = "admin";
	public static final String passWord2 = "123456";
	public static final String pathFile1 = "./src/main/resources/DataTest/CreateTicket.xls";
	public static final String pathFile2 = "./src/main/resources/DataTest/CreateDriver.xls";
	public static final String sheetName1 = "Create_Ticket";
	public static final String sheetName2 = "Create_DriverAccount";
	public static final String tableName = "Data Test";
	public static final String result_false = "false";
	public static final String result_pass = "pass";

}
